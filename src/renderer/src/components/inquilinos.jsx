...
export const Inquilinos = () => {
    ...
    const quitNot = () => {
        if (errEnv == 'visible') setErrEnv('invisible')
        if (envEx == 'visible') setEnvEx('invisible')
        if (errCampos == 'visible') setErrCampos('invisible')
        if (delEx == 'visible') setDelEx('invisible')
        if (actEx == 'visible') setActEx('invisible')
        if (errRep == 'visible') setErrRep('invisible')
    }
    ...
    const reloadTenants = async () => {
        ...
        const objTenant = []
        if (res.result == 1) {
            res.object.map(x => { objTenant.push(x.dataValues) })
    ...
    useEffect(() => { setSearchResult(registros) }, [registros])
    useEffect(() => {
            ...
            {showCrudModal && (
                <div className="crudOverlay" onClick={(e) => { if (e.target === e.currentTarget) { setShowCrudModal(false); limpiar(); } }}>
                    <div className="crudModalBox">
                        ...
                        <h2>{selectedID >= 0 ? 'Editar Inquilino' : 'Nuevo Inquilino'}</h2>
                        <div className="crudForm">
                            <input
                                type="text"
                                onChange={(e) => { setTenant({ ...tenant, fullName: e.target.value }); quitNot() }}
                            ...
                            <input
                                type='number'
                                placeholder="numero"
                                onChange={(e) => { setTenant({ ...tenant, number: e.target.value }); quitNot() }}
                                value={tenant.number}
                            />
                            <input
                                type='date'
                                placeholder="fecha de nacimiento"
                                onChange={(e) => { setTenant({ ...tenant, birthDate: e.target.value }); quitNot(); }}
                            />
                            <select
                                onChange={(e) => { setTenant({ ...tenant, maritalStatus: e.target.value }); quitNot() }}
                            ...
                            <input
                                placeholder="email"
                                type='email'
                                onChange={(e) => { setTenant({ ...tenant, email: e.target.value }); quitNot() }}
                                value={tenant.email}
                            />
                        </div>
                        <div className="buttonContainer">
                                ...
                                onClick={() => {
                                    ...
                                    if (tenant.fullName == "")
                                        ...
                                        (async () => {
                                            ...
                                            if (result != false) setEnvEx('visible')
                                        })()
                                    }
                                }}
                            />
                            <input type='button' value={'Modificar'}
                                onClick={() => {
                                    ...
                                    if (selectedID >= 0) {
                                        (async () => {
                                            const objTenant = { ...tenant, IDTenant: selectedID }
                                            const res = await updateTenant(objTenant);
                                            if (res != false) setActEx('visible');
                                            else setErrEnv('visible')
                                        })()
                                    }
                                }}
                            />
                                ...
                                onClick={() => {
                                    ...
                                    if (selectedID >= 0) {
                                        (async () => {
                                            ...
                                            if (res == true) { setDelEx('visible'); reloadTenants(); }
                                            else setErrEnv('visible')
                                        })()
                                    }
                                }}
                            />
                            <input type='button' value={'Limpiar'} onClick={() => { limpiar(); quitNot() }} />
                        </div>
